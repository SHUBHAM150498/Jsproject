...
form.onsubmit = function(e) {
    ...
    if(timeArray.length == 4) {
        if((parseInt(calcTime(str)) >= timeArray[0]) && (parseInt(calcTime(str)) < timeArray[1])) {
            ...
            document.getElementById("text1").innerText = "GOOD MORNING BUDDY!!!";
            document.getElementById('text2').innerText = "I hope your morning is as bright as your smile"
            document.getElementsByClassName("picture")[0].style.backgroundImage = "url(./assets/morning.jpg)"
    
        } else if((parseInt(calcTime(str)) >= timeArray[1]) && (parseInt(calcTime(str)) < timeArray[2])) {
            console.log("case2 passed");
            document.getElementById("text1").innerText = "LUNCH TIME BUDDY!!!";
            document.getElementById('text2').innerText = "Lunch is a meal eaten around the middle of the day."
            document.getElementsByClassName("picture")[0].style.backgroundImage = "url(./assets/lunch.jpg)"
    
        } else if((parseInt(calcTime(str)) >= timeArray[2]) && (parseInt(calcTime(str)) < timeArray[3])) {
            console.log("case3 passed");
            document.getElementById("text1").innerText = "GOOD EVENING BUDDY!!!";
            document.getElementById('text2').innerText = "Good evening is a little bit more difficult to say exactly what time it starts and ends, You can read more about “good evening” here.";
            document.getElementsByClassName("picture")[0].style.backgroundImage =  "url(./assets/lunch.jpg)";
    
        } else if((parseInt(calcTime(str)) >= timeArray[3]) || (parseInt(calcTime(str)) < timeArray[0])) {
            console.log("case4 passed");
            document.getElementById("text1").innerText = "GOOD NIGHT BUDDY!!!";
            document.getElementById('text2').innerText = "Good Night Sweet Dreams Take Care.";
            document.getElementsByClassName("picture")[0].style.backgroundImage = "url(./assets/night.jpg)";
